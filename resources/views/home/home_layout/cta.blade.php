    ...
    <div class="row">
      ...
      <div class="col-lg-6">
        <div class="lonyo-default-content lonyo-cta-wrap" data-aos="fade-up" data-aos-duration="700">
          ...
          <p
            class="editable-cta-description"
            contenteditable="{{ auth()->check() ? 'true' : 'false' }}"
            data-id="{{ $cta->id }}"
          >
            {{ $cta->description }}
...
<script>
  document.addEventListener("DOMContentLoaded", function() {
    ...
    function saveChanges(element) {
      ...
      let field = element.classList.contains("editable-cta-title") ? "title" : "description";
      let newValue = element.innerText.trim();

      fetch(`/update-cta/${ctaId}`, {
        method: "POST",
        headers: {
    ...
    document.querySelectorAll(".editable-cta-title, .editable-cta-description").forEach((el) => {
    ...
    // Upload Image
    let imageElement = document.getElementById('ctaImage')
